...
class MobileNetV2Model:
    ...
    def __init__(self):
        self.model = tf.keras.models.load_model('notebooks/egor_grishkov/model_MobileNetV2.keras')

    def predict(self, image_path):
        # Использование модели для предсказания класса изображения
        predictions = self.model.predict(MobileNetV2Model.__preprocess_image(image_path))
        # Возвращение индекса наиболее вероятного класса
        predicted_class_index = np.argmax(predictions[0])
        return dict_fish_classes.get(predicted_class_index)

    @staticmethod
    def __preprocess_image(image_path, target_size=(224, 224)):
